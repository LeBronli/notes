...
4. 拷贝赋值运算符
   ...
   1. 重载运算符本质上是函数，其名字由operator关键字后接表示要定义的运算符的符号组成
   
      `Foo& operator=(const Foo&);
   
   2. 为了与内置的赋值保持一致，赋值运算符通常返回一个指向其左侧运算对象的引用
   
   3. 合成拷贝赋值运算符
   
      1. 如果一个类未定义自己的拷贝赋值运算符，编译器会为他生成一个合成拷贝赋值运算符
      2. 对于某些类，这个用来禁止该对象的赋值
   
5. 析构函数

   1. 析构函数释放对象所用的资源，并销毁对象的非static成员
   2. 析构函数是类的一个成员函数
      1. 名字由波浪号接类名构成
      2. 没有返回值
      3. 不接受参数
      4. 例子：`~Foo();`
   3. 因为不接受参数，所以也不会被重载，一个类中只能有一个
   4. 析构函数和构造函数一样：有
      1. 初始化部分
      2. 函数体
   5. 与构造函数相反，构造函数中是先初始化成员，再执行函数体。但是在析构函数中，先执行函数体，在销毁成员，成员按照初始化顺序的逆序销毁
   6. 析构部分是隐式的，不需要控制成员如何销毁，成员销毁时发生什么完全依赖于成员的类型。
   7. 内置类型没有析构函数
   8. 无论何时，一个变量被销毁时，就会自动调用其析构函数
   9. 析构函数自身并不直接销毁成员，成员是在析构函数体之后隐含的析构阶段中被销毁的

6. 三五法则

   1. 通常，对于析构函数的需求比对拷贝函数或赋值运算符的需求更为明显
   2. 如果一个类需要析构函数，那么这个类肯定也需要一个拷贝构造函数和一个拷贝赋值函数
   3. 需要拷贝操作的类也需要赋值操作，反之亦然

7. 使用=default

   1. 可以通过将拷贝控制成员定义为=default来显式地要求编译器生成合成的版本
   2. 我们在类内使用=default时，合成的函数就是内联的，如果我们不想这样，那就要只对成员的类外定义使用=default，就像对拷贝赋值运算符所做的那样

8. 阻止拷贝

   1. 有些类不适合拷贝
   2. 但是不定义拷贝构造函数并没有用，因为编译器会自动合成
   3. 在新标准下，我们可以通过将拷贝函数和拷贝赋值运算符定义为**删除的函数**来组织拷贝
   4. **删除的函数**是这样一种函数：我们虽然声明了它们，但是不能以任何方式来使用它们
   5. 我们通过在参数列表后面加上=delete来指出我们希望将它定义为删除的
   6. `NoCopy(const NoCopy&) = delete;`
   7. =delete其实就是告诉编译器，我们不希望定义这些成员
   8. =delete必须出现在函数第一次声明时

9. 析构函数不能是删除的成员

   1. 因为如果可以是删除的话，就无法销毁此类型的对象了

   2. 如果一个类有某个成员删除了析构函数，我们就不能定义这个类的对象和临时对象。

   3. 对于删除了析构函数的对象，虽然我们不能定义这种类型的对象，但是我们可以动态分配这种类型的对象，只是我们不能释放它们：
      ...
      NoDtor nd; //错误，因为删除了析构函数
      NoDtor *p = new NoDtor(); //正确，但是我们不能使用delete p
      delete p; //错误，因为指向的对象的析构函数是删除的
      ```

10. 合成的拷贝控制成员可能是删除的

    1. 规则：
       1. 如果类的某个成员的析构函数是不可访问的（比如private的），则类的下面这些函数都被定义为删除的
          1. 合成析构函数
          2. 合成的拷贝构造函数
          3. 默认构造函数
       2. 如果类的某个成员的拷贝构造函数是删除的或者不可访问的，那类的合成拷贝构造函数是删除的
       3. 如果类的某个成员的拷贝赋值运算符是删除的或不可访问的，或者类有一个const或者引用成员，则合成拷贝赋值运算符被定义为删除的
       4. 如果类有引用成员，它没有类内初始化器，或者类有一个const成员，没有类内初始化器且类型未显式定义构造函数，则该类被定义为删除的
    2. 总结成一句话：如果类内有数据成员不能默认构造拷贝，复制或者销毁，则对应的成员函数将被定义为删除的
    3. 类内如果有成员有删除或者不可访问的析构函数，会导致其合成的默认和拷贝构造函数都是删除的，因为这是为了防止制造出无法销毁的对象

11. private拷贝控制

    1. 在新标准之前，也就是无法使用`delete`之前，类是通过`private`来阻止拷贝，也就是让其不可访问，从而达到删除的目的
    2. 将拷贝之类的函数定义为private之后，就只有友元和成员函数可以使用了
    3. 因为现在已经可以不用了，所以不深入了

12. 拷贝控制和资源管理

    1. 通常管理类外资源的类必须定义拷贝控制成员
    2. 一旦一个类需要析构函数，那么它几乎肯定需要拷贝构造函数和拷贝赋值运算符
    3. 有2中拷贝方法：
       1. 像一个值：完全拷贝，拷贝对象和被拷贝对象完全不相互影响了
       2. 像一个指针：共享底层数据，会相互影响
    4. 行为像值的类
       1. 为了完成这样的类，最少需要：
          1. 拷贝构造函数：完成成员的完全拷贝，而不只是拷贝指针
          2. 析构函数：释放成员（只释放动态内存中的，因为栈中的会自动释放）
          3. 拷贝赋值运算符：释放当前的成员，并接受右侧运算对象的成员
       2. 赋值运算符通常包含了析构函数+构造函数。但是顺序要注意，应该是先拷贝右侧运算对象的成员，然后再释放当前的成员。因为这样是异常安全的：即当运行过程中就算出现异常，也不会有太多破坏，如果先释放当前成员，如果中途发生异常，左侧对象就变成没有意义的状态了（即成员未定义）
    5. 行为像指针的类
       1. 
...
1. 核心思想是：
   1. 数据抽象，可以讲类的接口和实现分离
   2. 继承，可以定义相似的类型，并对其相似关系建模
   3. 动态绑定，可以一定程度上忽略相似类型的区别，以统一的方式使用它们
2. 基类将类型相关的函数和派生类不做改变直接继承的函数做了区分
3. 有些函数，基类希望他的派生类鸽子定义适合自己的版本，就会把他定义为**虚函数**，使用关键字`virtual`
4. 派生类必须通过使用**派生类列表**来明确指出它是由哪个或者哪些基类继承而来，形式是类名后面接冒号，然后再加上访问说明符和父类的名字
5. 派生类必须在其内部对所有重新定义的虚函数进行声明
6. 通过动态绑定，可以用同一段代码分别处理基类和派生类的不同对象，其实就是定义的时候用基类，然后使用的时候可以传入派生类，从而可以调用派生类自己定义的函数
7. 在C++中，当我们使用基类的引用调用一个虚函数的时候将发生动态绑定

##### 定义基类和派生类

1. 作为继承关系中根节点的类通常会定义一个虚析构函数

2. 基类必须把两种成员函数区分开来：

   1. 希望其派生类进行覆盖的函数
      1. 通常将其定义为虚函数
      2. 当我们使用指针或者引用调用虚函数时，该调用会被动态绑定
   2. 希望派生类直接继承不做改变的函数

3. 任何构造函数之外的非静态函数都可以是虚函数

4. 如果基类把一个函数声明为虚函数，那该函数在派生类中也默认是虚函数

5. 如果函数没有被定义为虚函数，那它的解析过程发生在编译时而不是运行时

6. 这里我感觉`virtual`其实就是告诉编译器，这个函数会被后面的派生类重载，具体使用哪个类定义的函数要看运行时传入了什么函数进来

7. 派生类可以继承基类来的成员，但是不一定有权限可以访问它们，比如派生类不能访问基类的`private`成员

8. 但是基类的`protected`成员是可以被派生类所访问的

9. 定义派生类

   1. 如果派生类没有重载基类中的某个虚函数，那会直接使用基类中的版本
...
10. 编译器会隐式地执行派生类到基类的类型转换

11. 派生类的构造函数同样是通过构造函数初始化列表来将实参传递给基类的构造函数

12. 如果基类定义了一个静态成员，那在整个继承体系中只存在该成员的唯一定义

13. 静态成员遵循通用的访问控制规则

14. 如果静态成员是可访问的，那我们既可以通过基类也能通过派生类访问它

15. 派生类的声明与普通类的声明没有区别，派生列表只是出现在定义的时候，而不是在声明的时候

16. 如果想要将某个类作为基类，那么这个类需要已经定义而不是仅仅声明

17. 有些类我们不希望继承它们，这时我们可以在类名后面跟上一个final声明符：`class No final {};`

18. 我们可以将指向基类的指针或者引用指向派生类，因为编译器会隐式转换

    1. 但是不存在从基类到派生类的自动类型转换，也就是说，18中的情况，反过来则不行
    2. 而且这种隐式转换只在引用或者指针中会出现，正常的对象不会出现

19. 我们要将对象或者表达式的静态类型和其动态类型区分开

##### 虚函数

1. 我们必须为每个虚函数提供定义，因为编译器也不知道到底会使用哪个
2. 当我们在派生类中覆盖了某个虚函数时，可以再次使用`virtual`关键字声明，但是其实也没必要，因为只要在基类中声明为虚函数，那所有派生类中该函数都是虚函数
3. 一个派生类覆盖基类的虚函数时，形参必须一模一样
4. 同样，派生类的虚函数返回类型也必须和基类一样，只有一个例外，那就是：
   1. 当虚函数返回类型是类本身的指针或引用时：如果D是由B派生得到，那么基类的虚函数可以返回B\*，而派生类的虚函数可以返回D\*
5. 派生类中如果定义了一个函数，与基类中虚函数的名字相同但是形参列表不同，这依然是合法的，编译器会认为这个函数与基类中的函数是独立的，也就是这个函数并不会覆盖基类中的虚函数。这个情况往往是不小心弄错了，但是这种很难发现，所以为了调试这个错误，我们可以使用`override`关键字来说明派生类中的虚函数，这样当我们没有覆盖响应的虚函数的时候，编译器会报错提醒我们
6. 我们还能使用`final`标志，这样覆盖操作会引起报错
7. 虚函数也可以使用默认实参，但是会造成一些问题，那就是：实参值会由调用的静态类型决定，也就是函数中定义的类型，而不会根据运行时实际传入的类型，会造成程序与我们语气不符
8. 有时我们希望虚函数不要进行动态绑定，而是强迫其执行某个版本，可以用作用域运算符来实现：
   1. `double undis = baseP->Quote::net_price(42)`这个表达式会强制使用`Quote`类型的虚函数版本
   2. 这种情况通常是：

##### 抽象基类

1. 纯虚函数与普通的虚函数不同，不需要定义，只需要在声明的时候写`=0`即可：`double net_price(std::size_t) const = 0`
2. 我们也可以为纯虚函数提供定义，但是必须要在类的外面
3. 含有纯虚函数的类是抽象基类，它们专门负责定义接口
4. 抽象基类不能创建对象，只能创建其派生类的对象，而且要覆盖所有的纯虚函数
5. 重构是非常普遍的现象，即重新设计类的体系以便于将操作和/或数据从一个类移动到另一个类中

##### 访问控制与继承

1. 派生访问说明符对于派生类的成员能否访问其直接基类的成员没什么影响，它的目的是控制派生类用户对于基类成员的访问权限

2. 派生类向基类的转换是否可访问由使用转换的代码决定，同时派生类的派生访问符也会有影响

3. 就像友元关系不能传递一样，友元关系也不能继承

4. 基类的友元在访问派生类时不具有特殊性

5. 同样的，派生类的友元也不能随意访问基类的成员

6. 有时我们需要改变派生类继承的某个名字的访问级别，通过使用`using`声明可以达到这个目的

   1. 这里派生用用`private`这个派生访问说明符，所以从基类中继承来的`size, n`都是派生类的私有成员，
   2. 这里使用`using`改变了其可访问性
   3. 派生类只能为那些它可以访问的名字提供`using`声明
...
7. 默认派生运算符也由定义派生类所用的关键字来决定：

   1. 使用`class`关键字的是默认私有继承
   2. 使用`struct`关键字是默认公有继承

8. `struct`和`class`唯二的差别就在于：
...
1. 派生类的作用域在基类之内，所以它才能像使用自己的成员一样使用基类
2. 搜索会从声明的类型开始：比如说如果将一个类型为基类的指针指向派生类对象，那么派生类中的所有成员方法都没有版本访问到，因为编译器会从基类开始搜索，而不是传入的派生类
3. 和其他作用域一样，派生类也能重新定义基类中的同名成员或函数，原理也和其他作用域一样，因为会从内层作用域开始搜索，所以就会自然而然地覆盖基类中定义的同名成员或函数<font color='red'>这里感觉还是有点奇怪的，那既然这样的话，虚函数是干什么用的？是为了避免2中的搜索起点带来的问题？</font>
4. 我们可以直接通过作用域运算符来指定作用域：`Base::mem`

##### 构造函数与拷贝控制

1. 基类通常应该定义一个虚析构函数，这样我们就能动态分配继承体系中的对象了，因为：
   1. 如果不是虚函数，而且又出现了指针类型为基类而实际类型为派生类的情况，会直接调用基类的析构函数，从而导致派生类中定义的新成员都无法被释放
2. 
...
1. 当我们希望将一些数据组合成单一对象，但是又不想麻烦地定义一个新数据结构来表示时，tuple是非常有用的

2. 定义与初始化

   1. 当我们定义tuple的时候，要指出每个成员的类型
   2. 标准库还提供了`make_tuple`函数，我们可以用它来生成`tuple`对象

3. 访问

   1. 我们要制定一个显式模板实参，它指出我们想要访问第几个成员

   2. 我们传递一个tuple对象，它返回指定成员的引用：`auto book = get<0>(item);`

      1. 尖括号中的必须是一个整型常量表达式

      2. 如果不知道具体的类型，可以用两个辅助类模板来查询tuple成员的数量和类型
